-- Modèle DBT pour DWH_MISSIONS_SANCTION (missions rattachées à des sanctions)
...
cross_miss_sui as (
    ...
    left join (
        select
            ...
            theme_decision,
            sous_theme_decision,
            case when type_de_decision in ('Injonction', 'Prescription', 'Saisine') then 'contrainte' else 'sans_contrainte' end as sanction,
            nombre
    ...
    group by
        ...
        d.type_de_decision, d.complement, d.theme_decision, d.sous_theme_decision, d.sanction
),
...
from missions_real_complet mrc
left join contrainte c on mrc.identifiant_de_la_mission = c.identifiant_de_la_mission
where mrc.statut_de_la_mission = 'Clôturé'